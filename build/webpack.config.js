...
module.exports = {
  ...
  resolve: {
    // 告诉webpack解析模块时应该搜索的目录，即require和import模块的时候，只写模块名的时候，去哪里找相关的模块，注意配置的路径必须为绝对路径
    modules: [path.resolve(__dirname, './src/'), "node_modules"],
    // 配置别名，类似于vue的@，也可以将整个路径全部配置为别名
    alias: {
      "@": path.resolve(__dirname, "./src/")
    },
    // 设置引入的模块，如果没有填写后缀名，webpack会自动添加后缀去查找，从左到右
    extensions: ["js", "vue"],
    //
  ...
  devServer: {
    ...
    hot: true,
    //即使hmr不生效浏览器也不自动刷新，css-loader，vue-loader，包括react，babel-preset内部都帮我们做了热更新处理，所以我们才可以配置完起效
    hotOnly: true
    // 代理
...
// 热更新文件需要我们写这部分代码，path为我们监听变化的文件，回调函数为监听变化后的处理
// if(module.hot){
